 ...
 */
@Service
public class LikeServiceImpl implements ILikeService {   // ★ 추가됨: 인터페이스 구현

    @Autowired
    private LikeDAO likeDAO;  // ★ 추가됨: DAO 주입
     ...
     */
    @Override
    @Transactional   // ★ 추가됨: INSERT/DELETE 동시 처리 보장
    public boolean toggleLike(String targetType, int targetId, String userId) {

        // 현재 좋아요 상태 확인
        boolean alreadyLiked = likeDAO.exists(targetType, targetId, userId);  // ★ 추가됨

        if (alreadyLiked) {
            // 이미 좋아요 누른 상태 → 좋아요 취소
            likeDAO.delete(targetType, targetId, userId);   // ★ 추가됨
            return false;   // 현재 상태 OFF
        } else {
            // 아직 안 눌렀으면 좋아요 추가
            likeDAO.insert(targetType, targetId, userId);   // ★ 추가됨
    ...
    public int getLikeCount(String targetType, int targetId) {
        return likeDAO.count(targetType, targetId);   // ★ 추가됨
    }
}
