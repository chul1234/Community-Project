app.controller('BoardStatsController', function ($scope, $http) {
    ...
    $http.get('/api/stats/posts').then(function (res) {
        ...
        // ✅ 추가: 오른쪽 여유 + 막대 끝 여유
        userChartOpt.layout = { padding: { top: 10, bottom: 10, left: 10, right: 30 } };
        userChartOpt.scales = { x: { grace: '5%' } };
        ...
        new Chart(document.getElementById('userDoughnut'), {
            ...
            data: {
                ...
                datasets: [
                    {
                        data: data.topUsers.map((v) => v.cnt),
                        backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#FF9F40', '#E7E9ED', '#76A346', '#0E3D59', '#8854D0'],
                        hoverOffset: 4,
                    },
                ],
            },
        ...
        // ✅ 추가: 오른쪽 여유 + 막대 끝 여유
        viewOpt.layout = { padding: { top: 10, bottom: 10, left: 10, right: 30 } };
        viewOpt.scales = { x: { grace: '5%' } };
